...
class P2VNet(nn.Module):
    ...
    def OTSU(self,tensor):
        ...
        for i in range(tensor.shape[0]):
            ...
            current_array = current_tensor.detach().cpu().numpy()

            # 使用大津阈值分割计算阈值
            threshold_value = threshold_otsu(current_array) + 0.1
            threshold_values[i] = threshold_value
            # threshold_values = [item - 0.1 for item in threshold_values]
    ...
    def forward(self, t1, t2, return_aux=True):
        ...
        mask = self.OTSU(mask)        # mask1 = (mask1 > 0.5) + 0.0 # (8,1,256,256)
        # mask = (mask > 0.5) + 0.0
